...
def insert_runner(testNum, probablityOfFaults):
    ...
    delay = datetime.now() - timer
    flag = False
    with open('ready_check.txt', 'r') as ready_file:
        l = ready_file.readline()
        if l == 'ready':
            flag = True
        ready_file.close()
    if flag:
        with open('info.txt', 'a') as f:
            f.write(str(delay.microseconds)+'\n')
            f.close()
    target = generate_target(select, vals)
    consistency_checker_insert(testNum, select, insert, before, after, target)
    return select, vals, target
...
def main():
    with open('ready_check.txt', 'w') as f:
        f.write('ready')
        f.close()
    ...
    for i in tqdm(range(numTests)):
        print(f'\n#{i+1}:\n')
        insertOrDelete = random.sample(['insert'], 1)[0]
        forceInsert = False
        ...
        if insertOrDelete == 'insert' or forceInsert:
            ...
            numInserts += 1
        last_time = datetime.datetime.now()
        row.append((datetime.datetime.now() - last_time))
