...
def sql_grammar(config: Config):
    ...
    SQL_GRAMMAR: Grammar = {
        '<Query>':
            ['SELECT <Select> FROM <FromList> WHERE <Condition>'],
       ...
       #     ['', 'UNION <Query>'],
        ...
        '<Condition>':
            ['<Comparison>', '<Condition> AND <Comparison>',
                '<Condition> OR <Comparison>', '<Comparison> <OrderBy>', '<Comparison> <GroupBy>'],

        '<OrderBy>':
            ['ORDER BY <Attribute>'],

        '<GroupBy>':
            ['GROUP BY <Attribute>', 'GROUP BY <Attribute> HAVING <Comparison>'],
